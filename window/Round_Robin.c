...
void print_rq(NODE* rq_head) { // 레디 큐 상태를 출력하는 함수
    printf("[");
    while (rq_head != NULL) {
...
int sched(PROC** running, NODE** rq_head, int total_procs, PROC* proc_arr) {
    ...
    if (*running) {
        ...
        if ((*running)->exec_time >= (*running)->size) { // 프로세스 실행 완료
            strcpy(terminated_proc_name, (*running)->name);
            (*running)->state = TERMINATED;
            proc_terminated = 1;
            *running = NULL;
        }
        else if ((*running)->time_slice <= 0) { // 시간 조각 소진
            (*running)->time_slice = TIME_SLICE; // 시간 조각 재설정
            (*running)->state = READY;
            insert_rq(rq_head, *running); // 레디 큐에 다시 삽입
            *running = NULL;
        }
    }
    ...
    if (*running == NULL) {
        *running = next_proc(rq_head);
        if (*running) {
            ...
            (*running)->time_slice = TIME_SLICE; // 시간 조각 재설정
            if (*running != last_running || proc_terminated) { // 프로세스 변경됨
